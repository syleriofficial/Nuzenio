...
alter table public.newsletter_subscribers add column if not exists country text default 'GLOBAL';
...
alter table public.newsletter_subscribers alter column country set default 'GLOBAL';
...
alter table public.user_preferences alter column preferred_country set default 'US';
...
alter table public.email_digest_logs alter column country set default 'GLOBAL';
...
alter table public.rss_sources alter column country set default 'GLOBAL';
...
alter table public.ai_briefs alter column country set default 'US';
...
create table if not exists public.trends (
  ...
  category text default 'top',
  country text default 'GLOBAL',
  momentum_score numeric(8,3) default 0,
  volume integer default 0,
  status text default 'emerging' check (status in ('emerging', 'growing', 'breaking', 'peak', 'declining')),
...
create table if not exists public.trend_snapshots (
  id bigint generated by default as identity primary key,
  trend_id text references public.trends(trend_id) on delete cascade,
  volume integer default 0,
  momentum_score numeric(8,3) default 0,
  country_breakdown jsonb default '{}'::jsonb,
  publisher_breakdown jsonb default '{}'::jsonb,
...
create table if not exists public.sentiment_scores (
  ...
  model_name text default 'nuzenio-heuristic-v1',
  metadata jsonb default '{}'::jsonb,
  created_at timestamptz default now()
);
...
create table if not exists public.entity_metrics (
  ...
  entity_type text default 'topic' check (entity_type in ('person', 'company', 'country', 'organization', 'product', 'event', 'topic')),
  country text default 'GLOBAL',
...
create table if not exists public.publisher_metrics (
  id bigint generated by default as identity primary key,
  ...
  average_freshness_minutes integer,
  coverage_categories text[] default '{}',
  source_diversity_score numeric(5,2) default 0,
  measured_at timestamptz default now(),
  metadata jsonb default '{}'::jsonb
);
...
create table if not exists public.alerts (
  id uuid primary key default gen_random_uuid(),
  user_id uuid references auth.users(id) on delete cascade,
  alert_type text not null check (alert_type in ('keyword', 'entity', 'country', 'topic')),
  query text not null,
...
create table if not exists public.saved_reports (
  ...
  user_id uuid references auth.users(id) on delete cascade,
  title text not null,
  report_type text default 'dashboard' check (report_type in ('dashboard', 'trend', 'entity', 'publisher', 'sentiment')),
...
alter table public.publisher_metrics enable row level security;
...
drop policy if exists "Entity metrics are publicly readable" on public.entity_metrics;
drop policy if exists "Publisher metrics are publicly readable" on public.publisher_metrics;
drop policy if exists "Users can manage own alerts" on public.alerts;
drop policy if exists "Users can manage own saved reports" on public.saved_reports;
drop policy if exists "Users can manage own shared dashboards" on public.shared_dashboards;
drop policy if exists "Users can read own dashboard exports" on public.dashboard_exports;
drop policy if exists "Users can insert own dashboard exports" on public.dashboard_exports;
drop policy if exists "Admins can manage trends" on public.trends;
drop policy if exists "Admins can manage trend snapshots" on public.trend_snapshots;
drop policy if exists "Admins can manage sentiment scores" on public.sentiment_scores;
drop policy if exists "Admins can manage entity metrics" on public.entity_metrics;
drop policy if exists "Admins can manage publisher metrics" on public.publisher_metrics;
...
on public.trend_snapshots for select using (true);

create policy "Sentiment scores are publicly readable"
on public.sentiment_scores for select using (true);

create policy "Entity metrics are publicly readable"
on public.entity_metrics for select using (true);

create policy "Publisher metrics are publicly readable"
on public.publisher_metrics for select using (true);
...
on public.saved_reports for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy "Users can manage own shared dashboards"
on public.shared_dashboards for all using (auth.uid() = owner_id) with check (auth.uid() = owner_id);

create policy "Users can read own dashboard exports"
on public.dashboard_exports for select using (auth.uid() = user_id);

create policy "Users can insert own dashboard exports"
on public.dashboard_exports for insert with check (auth.uid() = user_id);

create policy "Admins can manage trends"
on public.trends for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage trend snapshots"
on public.trend_snapshots for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage sentiment scores"
on public.sentiment_scores for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage entity metrics"
on public.entity_metrics for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage publisher metrics"
on public.publisher_metrics for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage alerts"
on public.alerts for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage saved reports"
on public.saved_reports for all using (public.is_admin()) with check (public.is_admin());

create policy "Admins can manage shared dashboards"
on public.shared_dashboards for all using (public.is_admin()) with check (public.is_admin());
...
create index if not exists trends_momentum_idx on public.trends(status, momentum_score desc, last_seen_at desc);
create index if not exists trends_country_idx on public.trends(country, category, momentum_score desc);
create index if not exists trend_snapshots_trend_idx on public.trend_snapshots(trend_id, snapshot_at desc);
create index if not exists sentiment_scores_article_idx on public.sentiment_scores(article_id, created_at desc);
create index if not exists sentiment_scores_entity_idx on public.sentiment_scores(entity_slug, sentiment_label, created_at desc);
create index if not exists entity_metrics_lookup_idx on public.entity_metrics(entity_slug, entity_type, measured_at desc);
create index if not exists entity_metrics_momentum_idx on public.entity_metrics(entity_type, momentum_score desc, measured_at desc);
create index if not exists publisher_metrics_lookup_idx on public.publisher_metrics(publisher_slug, measured_at desc);
create index if not exists publisher_metrics_country_idx on public.publisher_metrics(country, story_count desc, measured_at desc);
create index if not exists alerts_user_idx on public.alerts(user_id, alert_type, enabled);
create index if not exists alerts_query_idx on public.alerts(alert_type, enabled, created_at desc);
...
drop trigger if exists trends_touch_updated_at on public.trends;
...
drop trigger if exists shared_dashboards_touch_updated_at on public.shared_dashboards;
create trigger shared_dashboards_touch_updated_at
...
values
  ('Google News Global Top', 'top', 'https://news.google.com/rss?hl=en-US&gl=US&ceid=US:en', 'en', 'GLOBAL'),
  ('Google News Global World', 'world', 'https://news.google.com/rss/headlines/section/topic/WORLD?hl=en-US&gl=US&ceid=US:en', 'en', 'GLOBAL'),
  ('Google News US Local', 'local', 'https://news.google.com/rss/search?q=United%20States&hl=en-US&gl=US&ceid=US:en', 'en', 'US'),
  ('Google News UK Local', 'local', 'https://news.google.com/rss/search?q=United%20Kingdom&hl=en-GB&gl=GB&ceid=GB:en', 'en', 'GB'),
  ('Google News India Local', 'local', 'https://news.google.com/rss/search?q=India&hl=en-IN&gl=IN&ceid=IN:en', 'en', 'IN'),
  ('Google News YouTube Live Global', 'live', 'https://news.google.com/rss/search?q=site%3Ayoutube.com%2Fwatch%20live%20news%20channel&hl=en-US&gl=US&ceid=US:en', 'en', 'GLOBAL'),
  ('Google News YouTube Video Global', 'video', 'https://news.google.com/rss/search?q=site%3Ayoutube.com%20video%20news%20headlines&hl=en-US&gl=US&ceid=US:en', 'en', 'GLOBAL')
...
set enabled = false,
    status = 'disabled',
    updated_at = now(),
    last_error = 'Disabled by global starter-source cleanup; keep country-specific India local source instead.'
where url in (
  'https://news.google.com/rss?hl=en-IN&gl=IN&ceid=IN:en',
  'https://news.google.com/rss/headlines/section/topic/WORLD?hl=en-IN&gl=IN&ceid=IN:en',
  'https://news.google.com/rss/search?q=site%3Ayoutube.com%2Fwatch%20live%20news%20India&hl=en-IN&gl=IN&ceid=IN:en',
  'https://news.google.com/rss/search?q=site%3Ayoutube.com%20video%20news%20India&hl=en-IN&gl=IN&ceid=IN:en'
);
